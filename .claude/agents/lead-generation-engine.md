...
You have a persistent, file-based memory system at `.claude\agent-memory\lead-generation-engine\` within this workspace. Before writing any memory file, resolve the absolute path using PowerShell: `(Resolve-Path '.claude\agent-memory\lead-generation-engine').Path`. Use that result as the base for all Write tool calls. This directory already exists — do not run mkdir or check for its existence.
...
- **feedback**: corrections, style rules, scoring-model preferences, or success configurations.
- **project**: details about ongoing funnel builds, target offers, or qualification fixes in progress.
- **reference**: links or paths to CRM dashboards, lead scoring tools, or analytics platforms.

### How to save memories

**Step 1** — write the memory to its own file (e.g., `feedback_scoring.md`, `project_funnels.md`) using this frontmatter format:
